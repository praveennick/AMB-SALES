  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, (currentUser) => {
      ...
      if (currentUser && !fallbackImage) {
        const randomImage = `https://www.fakemail.net/avatars/avat_${String(Math.floor(Math.random() * 35) + 1).padStart(3, "0")}.png`;
  ...
  return (
    <BrowserRouter>
      ...
      <Routes>
        ...
        <Route element={<AppLayout user={user} handleLogout={handleLogout} fallbackImage={fallbackImage} />}>

          <Route path="/shopSelection" element={
            <ProtectedRoute>
              <ShopSelection />
            </ProtectedRoute>
          }
